...
document.addEventListener('input', (e) => {
  ...
  typingTimer = setTimeout(() => {
    ...
    fetchSuggestion(target, liveValue, location.href, getFieldType(target));
  }, typingDelay);
}, { capture: true });
...
async function fetchSuggestion(target, text, url, fieldType) {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({ type: "GET_COMPLETION", text, url, fieldType }, (res) => {
        if (chrome.runtime.lastError) {
          ...
          // Context invalidated = extension was reloaded; page needs a refresh — fail silently
    ...
    if (myId !== requestId || currentTarget !== target) return;
    ...
    console.error('[Wryte] error:', err.message);
  }
}
